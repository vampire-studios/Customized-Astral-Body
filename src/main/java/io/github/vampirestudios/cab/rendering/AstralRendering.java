...
public class AstralRendering {
    ...
    public static void renderCustomAstralBody(MatrixStack matrixStack, MinecraftClient client, Dimension dimension, ClientWorld world, VertexBuffer lightSkyBuffer,
        ...
        if (dimension.hasVisibleSky()) {
            ...
            Vector3f vector3f = ((AstralBodyModifier)dimension).getSunTint();
            textureManager.bindTexture(SUN);
            bufferBuilder.begin(7, VertexFormats.POSITION_COLOR_TEXTURE);
            bufferBuilder.vertex(matrix4f2, -t, 100.0F, -t).color(vector3f.getX(), vector3f.getY(), vector3f.getZ(), 1.0F).texture(0.0F, 0.0F).next();
            bufferBuilder.vertex(matrix4f2, t, 100.0F, -t).color(vector3f.getX(), vector3f.getY(), vector3f.getZ(), 1.0F).texture(1.0F, 0.0F).next();
            bufferBuilder.vertex(matrix4f2, t, 100.0F, t).color(vector3f.getX(), vector3f.getY(), vector3f.getZ(), 1.0F).texture(1.0F, 1.0F).next();
            bufferBuilder.vertex(matrix4f2, -t, 100.0F, t).color(vector3f.getX(), vector3f.getY(), vector3f.getZ(), 1.0F).texture(0.0F, 1.0F).next();
            bufferBuilder.end();
            BufferRenderer.draw(bufferBuilder);
            ...
            double d = Objects.requireNonNull(client.player).getCameraPosVec(f).y - world.getSkyDarknessHeight();
            if (d < 0.0D) {
                matrixStack.push();
